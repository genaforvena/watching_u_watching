...
2.  **Run the Spectre Probe:**
    ...
    -   **Baseline Mode:** To measure the model's baseline behavior without any transformations or memory manipulation, add the `--baseline` flag:
        ```bash
        python src/audits/cryptohauntological_probe/probe_runner.py --llm_api ollama --model_name llama3.2:latest  --swap_type zy --initial_word cozy --num_rounds 50 --baseline
        ```
